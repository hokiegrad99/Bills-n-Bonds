...
import { HOLDING_COLUMNS, SAMPLE_CSV, parseHoldingsCSV } from '../../lib/csv';

interface ImportDialogProps {
  open: boolean;
  onClose: () => void;
  onImport: (rows: ReturnType<typeof parseHoldingsCSV>['rows']) => number;
}

export function ImportDialog({ open, onClose, onImport }: ImportDialogProps) {
  const [text, setText] = useState('');
  const [result, setResult] = useState<ReturnType<typeof parseHoldingsCSV> | null>(null);
  ...
  function handleFile(file: File) {
    ...
    reader.onload = () => {
      ...
      setText(value);
      setResult(parseHoldingsCSV(value));
    };
  ...
  function handleCommit() {
    ...
    setResult(null);
    onClose();
    // Lightweight UX feedback (parent will re-render with new holdings).
    void added;
  }
    ...
    <Modal open={open} onClose={onClose} title="Import Holdings from CSV" size="lg">
      <div className="space-y-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  a.href = url;
                  a.download = 'bnb-holdings-sample.csv';
                  a.click();
        ...
        <details>
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              if (e.target.value.trim()) setResult(parseHoldingsCSV(e.target.value));
              else setResult(null);
            }}
          />
        </details>
        ...
        {result && (
          <div className="card p-3 text-sm">
            <div className="flex items-center justify-between">
              <div>
                <strong>{result.rows.length}</strong> holding{result.rows.length === 1 ? '' : 's'} ready to import.
              </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          >
            Import {result?.rows.length ?? 0} holding{result?.rows.length === 1 ? '' : 's'}
          </button>
        </div>
      </div>
    </Modal>
